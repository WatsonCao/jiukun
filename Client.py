        ...
        with self.is_any_updated_lock:
            ...
            if self.market_log!=None:
                str1+="PreBalance:{:.4f}\t\tCurrMargin:{:.4f}\t\tAvailable:{:.4f}\nFloatProfit:{:.4f}\t\tCloseProfit:{:.4f}\n".format(
                    self.market_log["PreBalance"],self.market_log["CurrMargin"],self.market_log["Available"],self.market_log["FloatProfit"],self.market_log["CloseProfit"])
                str1 += "TotalMMCount:{:.4f}\t\tMMCompleteCount:{:.4f}\t\tTotalOptionTradeCount:{:.4f}\n".format(
                    self.market_log["TotalMarketMakingCount"], self.market_log["TotalMarketMakingCompleteCount"], self.market_log["TotalOptionTradeCount"],)

            self.logger.info(str1)
            self.market_data_updated_lock.release()
            pass
